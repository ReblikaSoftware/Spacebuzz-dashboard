...
public class WebSocketAndVideoControl : MonoBehaviour
{
    ...
    // C:/Users/31653/Downloads/SB_VR_06_03_7200x3600_injected.mp4
    ...
    void TryReconnect()
    {
        ...
        {
            websocketClient.Start(); // Assuming Start() attempts to connect or reconnect
        }
        else
        {
            // Initialize your WebSocket client if it's null
            InitializeWebSocket();
        }

        // Optionally, reset isReconnecting after a delay
        StartCoroutine(ResetReconnectionFlag());
    }

    IEnumerator ResetReconnectionFlag()
    {
        // Wait for a few seconds before allowing another reconnection attempt
        yield return new WaitForSeconds(10); // Adjust based on your needs
    ...
    void Update()
    {
        ...
        // Check if the application is active and the WebSocket client is not connected.
        if (Application.isPlaying && (websocketClient == null || !websocketClient.IsRunning) && !isReconnecting)
        {
            TryReconnect();
        }
